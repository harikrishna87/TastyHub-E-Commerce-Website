...
const UserOrders: React.FC = () => {
  ...
  const handleConfirmCancelOrder = async () => {
    ...
    if (!finalReason.trim()) {
      messageApi.error('Please select or enter a cancellation reason.');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        messageApi.success('Your order has been cancelled successfully.');
      ...
      messageApi.error(err.response?.data?.message || 'Failed to cancel the order.');
    }
  };
  ...
  const handleRetryOrder = async (orderId: string) => {
    try {
      ...
      messageApi.error(err.response?.data?.message || 'Failed to restart search.');
    } finally {
  ...
  const getStatusTag = (status: OrderDeliveryStatus) => {
    switch (status) {
      ...
      case 'Refunded':
        return (
          <span style={{ background: '#f0f5ff', color: '#2f54eb', border: '1px dashed #adc6ff', borderRadius: '5px', padding: '2px 10px', display: 'inline-flex', alignItems: 'center', gap: '4px', fontWeight: 600 }}>
            <i className="pi pi-undo" /> Refunded
          </span>
        );
      default:
        return (
              ...
              <div className="cancellation-deadline-card">
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{
                    ...
                    <i className="pi pi-clock" style={{ fontSize: '1.1rem' }} />
                  </div>
                  <div>
                    <h4 style={{ margin: 0, fontWeight: 700, color: '#92400e', fontSize: '0.92rem' }}>10-Min Cancellation Window Active</h4>
                    <p style={{ margin: '2px 0 0 0', fontSize: '0.78rem', color: '#b45309', lineHeight: 1.3 }}>You can cancel and get a full refund.</p>
                  </div>
                </div>
                
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                  <CancelCountdown createdAt={selectedOrderForStatus.createdAt} onExpire={fetchUserOrders} />
                  <Button 
                    label="Cancel" 
          ...
          <div className="delivery-scoped-font" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingTop: '10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <Dropdown
                ...
                onChange={(e: any) => setCancelReasonOption(e.value)}
